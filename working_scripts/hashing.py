...
#### working variables
location= 'http://localhost/data/working_scripts/json.php'# this variable is to fetch the information from php script #content in this variable must have quotation marks
path = 'C:\wamp64\www\data\\file_test' #changing this variable will change the location of the file produced by this script
...
#had to encode with utf-8 to be able to use the hash library####################################################
	...
	if obj['data'][i]['first_name'] != None:  #conditional is set to catch any None vaules that might be in the json and will convert accordingly 
		...
		f_name = obj['data'][i]['first_name'] =="null"
		f_name = obj['data'][i]['first_name'].encode('utf-8')
		#### end of if 
		### start of if statement
	if obj['data'][i]['last_name'] != None:  #conditional is set to catch any None vaules that might be in the json and will convert accordingly 
		...
		f_name = obj['data'][i]['last_name'] =="null"
		l_name = obj['data'][i]['last_name'] =="null"
		#### end of if 
		### start of if statement
	if obj['data'][i]['first_name'] != None:  #conditional is set to catch any None vaules that might be in the json and will convert accordingly 
